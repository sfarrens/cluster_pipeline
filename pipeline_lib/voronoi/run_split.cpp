/*
	...
	Version:		1.1
	Last changes:
		- command line is changed, please he output_help
		- No default output filenames are included, so ensure the pipeline save
			the split output in the right file/box and that those files get updated instead of being recreated
		- cout only outputs the results now, unless it's an error/exit
	Description: 	Code to split data points into a single box/file in xyz space.
