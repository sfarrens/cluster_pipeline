/*
	...
	Version:		1.1
	Last changes:	
		- No output filenames are included, not even for the .splitn sub-outfiles in the boxes file
		- If an output filename is not given (what it used to be the 'boxes' files, output is to cout
		- cout only outputs the results now, unless it's an error/exit
	Description: 	Code to split a xyz space into boxes, from a list of object positions/files.
					It must be run *before* run_split. Requires the 'bound' file.
	Compile:		
...
void output_help(){
	cout<<"\nUsage:\n"<<flush;
	cout<<"./main_split -i <xyz_file_1> ... <xyz_file_n> -b <file_boundary> (-inters <perc>) (-n <N>) (-o <file_out>)\n\n"<<flush;
	cout<<"<xyz_file_1>...<xyz_file_n> are the input catalogs\n"<<flush;
	...
	cout<<"<N> is the average number of data points in each box/file (optional, default="<<DEFAULT_NUM_MAX_CUT<<")\n\n"<<flush;
