...
parser.add_option("--min_rich_bin", dest = "min_rich_bin", default = 0.0, type = "float",
                  help = "Minimum richness value allowed. [Default 0.0]")
parser.add_option("--max_rich_bin", dest = "max_rich_bin", default = -1, type = "float",
                  help = "Maximum richness value allowed. [Default maximum value in list]")
...
cluster = np.genfromtxt(opts.cluster_file, dtype="S", unpack = True)[opts.c_cols]
dtypes = [('id', 'i4'), ('ra', 'f8'), ('dec', 'f8'), ('z', 'f8'), ('rich', 'f8'), ('sn', 'f8')]
...
if opts.max_rich_bin < 0: opts.max_rich_bin = max(max(cluster.rich), max(mock.rich))
...
index = ((mock.z >= opts.min_z_bin) & (mock.z  <= opts.max_z_bin) &
                 (mock.rich >= opts.min_rich_bin) & (mock.rich <= opts.max_rich_bin) &
...
index = ((cluster.z >= opts.min_z_bin) & (cluster.z <= opts.max_z_bin) &
         (cluster.rich >= opts.min_rich_bin) & (cluster.rich <= opts.max_rich_bin))
...
g_bin_2 = np.zeros(n_rich_bins).astype('float')
...
for i in range(mock.size):
    ...
    if m_match_flag[i] > -1:
        print>> match_out, '       Y ', '%11.3f' % mock.dist[i], '   %-8i' % cluster.id[m_match_flag[i]],
...
for i in range(cluster.size):
    print>> c_match_out, '%-8i' % cluster.id[i], '%6.3f' % cluster.ra[i], '%+7.3f' % cluster.dec[i], '%7.3f' % cluster.z[i],
...
im = ax.imshow(complete_bin, extent = [opts.min_z_bin, opts.max_z_bin, opts.min_rich_bin, opts.max_rich_bin], aspect = 'auto',
                 interpolation = interpolation, cmap = colormap, norm = normalisation)
ax.set_xlabel('z')
ax.set_ylabel('log$_{10}$ N$_{true}$')
...
ax = fig.add_subplot(gs[0])
cax = fig.add_subplot(gs[1])
im = ax.imshow(pure_bin, extent = [opts.min_z_bin, opts.max_z_bin, opts.min_rich_bin, opts.max_rich_bin], aspect = 'auto',
                 interpolation = interpolation, cmap = colormap, norm = normalisation)
ax.set_xlabel('z')
ax.set_ylabel('log$_{10}$ N$_{obs}$')
cbar = fig.colorbar(im, cax=cax)
...
gs = gridspec.GridSpec(1, 1)
ax = fig.add_subplot(gs[0])
ax.plot(x_rich_vals, g_bin_2, linewidth=1.0, c='b')
ax.set_ylim([0.0, 1.0])
...
ax = fig.add_subplot(gs[0])
cax = fig.add_subplot(gs[1])
im = ax.imshow(g_bin, extent = [opts.min_z_bin, opts.max_z_bin, opts.min_rich_bin, opts.max_rich_bin], aspect = 'auto',
                 interpolation = interpolation, cmap = colormap, norm = normalisation)
ax.set_xlabel('z')
...
if opts.make_mock_plots:
    ...
    #Mass vs. N_true_mock
    heat, yedges, xedges = np.histogram2d(mock.rich, mock.mass, bins=(n_rich_bins, n_mass_bins))
    heat = np.log10(heat)   
    fig3 = plt.figure()
    ...
    im = ax31.imshow(np.flipud(heat), extent = [opts.min_mass_bin, opts.max_mass_bin, opts.min_rich_bin, opts.max_rich_bin], aspect = 'auto',
    ...
    #z vs. N_true_mock
    heat, yedges, xedges = np.histogram2d(mock.rich, mock.z, bins=(n_rich_bins, n_z_bins))
    heat = np.log10(heat)   
    fig4 = plt.figure()
    ...
    im = ax41.imshow(np.flipud(heat), extent = [opts.min_z_bin, opts.max_z_bin, opts.min_rich_bin, opts.max_rich_bin], aspect = 'auto',
                    interpolation = interpolation, cmap = colormap)
    ax41.set_xlabel('z')
